...
from GPErks_library.gsa_parameters_ranking import correct_S
from GPErks_library.file_utils import read_ionic_output
...
def plot_pairwise_waves(XL, 
    ...
    for t, ax in enumerate(axes.flatten()):
        ...
        if xlabels is not None:
            if i == 0:
                ax.set_ylabel(xlabels[j], fontsize=24,rotation='vertical', labelpad=10)
                ax.set_yticks([])
            if j == in_dim - 1:
                ax.set_xlabel(xlabels[i], fontsize=24,rotation='horizontal', labelpad=10)
                ax.set_xticks([])
            if i == in_dim - 1 and j == 0:
                ax.legend(handles, labels, loc="center", fontsize=24 )
    # plt.tight_layout()
...
def plot_waves_paramSpace(waves_folder,
    ...
    evenly_spaced_interval = np.linspace(0.1, 1, Ncolors)
...
def plot_R2_HM(loadpath,
    ...
    ylabels = read_labels_from_file(dataFolder+"/ylabels.txt")
...
def plot_wave_full(wave_file, xlabels=None, display="impl", filename="./wave_impl"):
    ...
    newcmp = ListedColormap(newcolors, name='winterspring')
    ...
    for k in range(input_dim * input_dim):
        ...
        if i > j:
            ...
            if i == input_dim - 1:
                axis.set_xlabel(xlabels[j], **afont, fontsize=24,rotation='horizontal')
                axis.set_xticklabels([])
            else:
                axis.set_xticklabels([])
            if j == 0:
                axis.set_ylabel(xlabels[i], **afont, fontsize=24,rotation='vertical', labelpad=10)
...
def plot_wave_X_subset(wave_file, 
    ...
    for k in range(W.input_dim * W.input_dim):
        ...
        if i > j:
            ...
            if i == W.input_dim - 1:
                axis.set_xlabel(xlabels[j], fontsize=24)
                axis.set_xticklabels([])
            else:
                axis.set_xticklabels([])
            if j == 0:
                axis.set_ylabel(xlabels[i], fontsize=24)
                axis.set_yticklabels([])
            else:
                axis.set_yticklabels([])
    ...
    cbar = fig.colorbar(im, cax=cbar_axis)
    cbar.set_label(cbar_label, size=24)
    fig.tight_layout()
    plt.savefig(filename + ".png", bbox_inches="tight", dpi=300)
...
def plot_inputspace(xlabels,
    ...
    parameters = xlabels
    if gpe_mode=="full":
            loadpath = loadpath + "output/wave" + str(waveno) + "/" + str(0) + "/"
            data = np.loadtxt(loadpath + "X_train.txt", dtype=np.float64)
        
    elif gpe_mode=="best":
        loadpath = loadpath + "output/" +  str(0) + "/best_split/"
        data = np.loadtxt(loadpath + "X_train.txt", dtype=np.float64)
    
    elif gpe_mode=="orig":
        data = np.loadtxt(loadpath + "data/wave" + str(waveno) + "/" + "X.txt", dtype=np.float64)
    ...
    cbar_label = 'Number of NROY points'
    for k in range(input_dim * input_dim):
        i = k % input_dim
        j = k // input_dim
        ...
        if i > j:
            ...
            if i == input_dim - 1:
                axis.set_xlabel(parameters[j], fontsize=12, rotation='vertical')
            else:
                axis.set_xticklabels([])
            if j == 0:
                axis.set_ylabel(parameters[i], fontsize=12, rotation='horizontal', labelpad=100)
            else:
                axis.set_yticklabels([])
...
def plot_param_ranges(initial_X_file,
                     waves_list, 
                     xlabels, 
                     idx_param=None,
                     filename="./wave_param_ranges",
                     figsize=None):
    
    print('-----------------------------------------')
    print("Plotting parameter ranges for:")
    for x in xlabels:
        print(x)
    print('-----------------------------------------')

    X_init = np.loadtxt(initial_X_file,dtype=float,usecols=idx_param)

    init_ranges = np.column_stack((np.min(X_init,axis=0),
                                   np.max(X_init,axis=0)))

    dct = {}
    dct["init"] = init_ranges
    ...
    if len(xlabels)>=4:
        ...
        ncols = len(xlabels)
    nrows = math.ceil(idx_param.shape[0]/ncols)

    for n in range(len(waves_list)):
        ...
        param_ranges = np.column_stack((np.min(NIMP,axis=0),np.max(NIMP,axis=0)))
        dct["wave"+str(n+1)] = param_ranges

    img_width = 10

    if figsize is None:
        figsize = (img_width,img_width/4*nrows)

    fig,ax = plt.subplots(nrows,ncols,figsize=figsize)
    ax = ax.flatten()

    keys = list(dct.keys())
    wave_labels = ["wave"+str(i+1) for i in range(len(waves_list))]
    wave_labels = ["initial"]+wave_labels

    evenly_spaced_interval = np.linspace(0, 1, len(wave_labels))
    colors = [cm.coolwarm(x) for x in evenly_spaced_interval]
    ...
    for i in range(len(ax)):
        ...
        if i < len(xlabels):
            for j,k in enumerate(keys):
                ax[i].plot([j,j],dct[k][i,:],color=colors[j])   

            ax[i].set_ylabel(xlabels[i])

            ax[i].set_xlim([-1,len(wave_labels)])
            ax[i].set_xticks(range(len(wave_labels)))
            ax[i].set_xticklabels(wave_labels,rotation=90)
