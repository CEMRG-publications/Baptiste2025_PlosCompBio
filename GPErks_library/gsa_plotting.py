...
def gsa_heat(ST, 
    ...
    if xlabels_latex is None:
        h1.set_xticklabels(xlabels, rotation=rot_angle_x, va="top",fontsize=fontsize,**afont)
    else:
        h1.set_xticklabels(xlabels_latex, rotation=rot_angle_x, va="top",fontsize=fontsize, **afont)
    
    if ylabels_latex is None:
        h1.set_yticklabels(ylabels, rotation=rot_angle_y, ha="right",fontsize=fontsize,**afont)
    else:
        h1.set_yticklabels(ylabels_latex, rotation=rot_angle_y, va="top",fontsize=fontsize, **afont)
    ...
    ht.set_xticks(np.arange(df.shape[1])+0.5)
    if xlabels_latex is None:
        ht.set_xticklabels(xlabels, rotation=rot_angle_x, va="top",fontsize=fontsize,**afont)
    else:
        ht.set_xticklabels(xlabels_latex, rotation=rot_angle_x, va="top",fontsize=fontsize,**afont)
    
    if ylabels_latex is None:
        ht.set_yticklabels(ylabels, rotation=rot_angle_y, ha="right",fontsize=fontsize,**afont)
    else:
        ht.set_yticklabels(ylabels_latex, rotation=rot_angle_y, va="top",fontsize=fontsize, **afont)
    ...
    plt.tight_layout()
    plt.savefig(savepath + img_name, bbox_inches="tight", dpi=300)
...
def plot_rank_land_comparison(loadpath,
    ...
    index_i = read_labels_from_file(loadpath[0] + "data/xlabels.txt")
    label = read_labels_from_file(loadpath[0] + "data/ylabels.txt")
...
def plot_rank_GSA(datapath,
    ...
    # assumes that xlabels and ylabels are the same for all tests
    index_i = read_labels_from_file(datapath + "/xlabels.txt")
    ...
    plt.xticks(x+barWidth, index_i_sorted, rotation=90,fontsize=fontsize,**afont)
    ax.tick_params(axis='both',labelsize=fontsize,**afont)

    cutoff_param = np.where(np.array(bars_sorted_sum)>0.90)[0][0]
    if color_important is None:
        color_important = color[0]
    ...
    #     plt.rc('text', usetex=False)
    if criterion=='STi':
        ax.set_ylabel('Sensitivity',fontsize=fontsize,**afont)
    else:
...
def plot_rank_GSA(datapath,
    ...
    cutoff_param = np.where(np.array(bars_sorted_sum)>0.80)[0][0]
    if color_important is None:
        color_important = color[0]
    color_unimportant = "lightgray"
    colors = [color_important,]*(cutoff_param+1)+[color_unimportant,]*(len(bars_sorted_sum)-cutoff_param-1)
