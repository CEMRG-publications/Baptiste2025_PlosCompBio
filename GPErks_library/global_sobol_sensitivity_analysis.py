...
def global_sobol_sensitivity_analysis(loadpath,
    ...
    if sample_method=='sobol':
        ...
        if uncertainty:
            ...
            gsa = SobolGSA(dataset, n=n, seed=seed)
            gsa.estimate_Sobol_indices_with_emulator(emulator, n_draws=n_draws)
            ...
            np.savetxt(savepath + "/STi.txt", gsa.ST, fmt="%.6f")
            np.savetxt(savepath + "/Si.txt", gsa.S1, fmt="%.6f")
            np.savetxt(savepath + "/Sij.txt", gsa.S2, fmt="%.6f")

            np.savetxt(savepath + "STi_std.txt", gsa.ST_std, fmt="%.6f")
            np.savetxt(savepath + "Si_std.txt", gsa.S1_std, fmt="%.6f")
            np.savetxt(savepath + "Sij_std.txt", gsa.S2_std, fmt="%.6f")
            ...
            ST_std = T_Si["ST_conf"].reshape(1, -1) / DEFAULT_GSA_Z
            S1_std = first_Si["S1_conf"].reshape(1, -1) / DEFAULT_GSA_Z    

            if calc_second_order:
                S2_std = np.array(second_Si["S2_conf"]).reshape(1, -1) / DEFAULT_GSA_Z
            else:
